...
is the query named "GetDelivery",
which can be accessed via the [ExampleConnector.getDelivery] property.


An example of the property for a mutation
is the mutation named "CreateDelivery",
which can be accessed via the [ExampleConnector.createDelivery] property.
...
For example, the "GetUser" query has no variables
and can be executed via the
[ExampleConnector.getUser]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.getUser.execute()
println("GetUser query returned: ${queryResult.data}")
```
...
For example, the "GetDelivery" query has 1 required variable ("id")
and can be executed via the [ExampleConnector.getDelivery]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.getDelivery.execute(id=java.util.UUID.randomUUID())
println("GetDelivery query returned: ${queryResult.data}")
```
...
For example, the "CreateStore" mutation has no variables
and can be executed via the
[ExampleConnector.createStore]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.createStore.execute()
println("CreateStore mutation returned: ${mutationResult.data}")
```
...
For example, the "CreateOrderItem" mutation has 2 required variables ("orderId" and "productId")
and can be executed via the [ExampleConnector.createOrderItem]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.createOrderItem.execute(orderId=java.util.UUID.randomUUID(), productId=java.util.UUID.randomUUID())
println("CreateOrderItem mutation returned: ${mutationResult.data}")
```
